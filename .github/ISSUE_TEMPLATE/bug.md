---
name: Bug report
about: Report a problem with specwright on Claude Code or Codex
title: "[bug] "
labels: ["bug"]
---

## Component affected

Name the shared workflow (`init`, `change`, `plan`, `implement`, `pr`, `review`,
`ship`, or `delivery`), a role profile, the scaffolder or a validator, or the
package surface. Include whether you invoked `/sw:*` in Claude Code, `$sw:*` in
Codex, or natural language.

## Host and environment

- Host: Claude Code | Codex
- Host version:
- Operating system and filesystem:
- Plugin version reported by `claude plugin list --json`, `codex plugin list --json`, or the installed Codex manifest:
- Project mode, when relevant: shared | local

## Reproduction steps

1.
2.
3.

## Expected behavior

What should have happened.

## Actual behavior

What happened. Include the exact diagnostic and a redacted log or screenshot when
useful. Never include credentials.

## Anything else

Additional context, related issues, or a hypothesis about the cause.
