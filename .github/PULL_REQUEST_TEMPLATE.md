...
- [ ] (For validator changes) Ran `UV_CACHE_DIR=/tmp/specwright-uv-cache uv run --offline --with PyYAML python3 tests/skills/test_validation.py` and `bash tests/validate-change/run.sh`.
- [ ] (For package changes) Ran `claude plugin validate --strict plugins/sw` and the relevant `tests/install/run.sh` groups.
- [ ] (For host package changes) Ran `bash tests/release/run.sh`; native Codex ingestion ran in a disposable environment.
- [ ] Tried the skill end-to-end in an agent session (Claude Code, Codex, or whichever you use) and observed the expected behavior.
...
- [ ] `.specwright/` edits, if any, are limited to the current dogfooded change/delivery artifacts; unrelated historical artifacts are unchanged.
