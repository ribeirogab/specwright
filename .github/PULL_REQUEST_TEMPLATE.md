...
- [ ] (For modified skills) Ran `python skills/sw/scripts/quick_validate.py <skill-path>` — output was `Skill is valid!`.
- [ ] (For modified skills) Ran `python skills/sw/scripts/package_skill.py <skill-path> /tmp` — output ended with `Successfully packaged skill to: /tmp/<skill-name>.skill`.
...
- [ ] No edits under `.specwright/`, `.agents/`, or `.claude/` (maintainer-local dirs, out of scope).
