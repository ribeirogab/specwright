...
If you find a security issue in memex or its bundled companions, please report it privately by emailing **gblosr@gmail.com**. Do not open a public issue or pull request that describes the vulnerability — that exposes other users before a fix is available.

When reporting, please include:

- The affected component (the `memex` skill, a bundled companion, or a script).
...
- The `memex` skill under [`skills/memex/`](skills/memex/) — the published surface that `npx skills add` installs, plus its bundled companions under `.agents/skills/memex-*/`.
...
- The `.agents/`, `.claude/`, and `.vault/` directories — maintainer-local content (dogfooded memex output and personal knowledge vault). Not consumed by `npx skills add` and not part of the published skill surface.
