---
name: sw-new-pr
description: "Open the pull request for the current spec, following specwright PR conventions — resolves branch/base, pushes if needed, fills the repo PR template, English Conventional-Commit title and body, no AI attribution. Use as the final delivery step of the spec flow, or whenever the user asks to open a PR."
---
...
- **Body** — English (committed artifacts are English). Top: ONE product-level sentence (no filenames, no pipeline mechanics) describing WHAT the PR does, then 2–5 concrete bullets (resource + action). For a spec-driven PR, fill `## Summary` with context/decisions and link the spec artifacts as absolute GitHub URLs on the branch:
  - design: `design.md`, spec: `spec.md`, tasks: `tasks.md`.
