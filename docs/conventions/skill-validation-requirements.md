...
every host adapter. Validation failures block release even when one host happens to
accept the file.
...
Package-surface changes also run `bash tests/install/run.sh` and the host
release smoke test.
