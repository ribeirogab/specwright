...
2. **Spec-document-reviewer subagent** — dispatch it (see the sibling `spec-document-reviewer-prompt.md`) over `issue.md` + `spec.md` + `tasks.md`. Route it via `.specwright/models.md` (role `spec-reviewer`); inherit if your agent has no binding, and treat its `effort` as advisory. Fix, re-dispatch until Approved (max 3 iterations, then surface to the human).
