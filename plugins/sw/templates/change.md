...
> The ticket: the approved *why*, the acceptance criteria, and the change's status. `status:` lives **only** here — `pending | in-progress | shipped | blocked` — and `shipped:` gets the ship date when the PR is open and ```sw:review``` reached `lgtm`. The technical *how* (architecture and tasks) lives in the sibling `plan.md`, written by ```sw:plan```. Delivery membership is data: `delivery:` holds the parent delivery folder, or `null` for a standalone change.
