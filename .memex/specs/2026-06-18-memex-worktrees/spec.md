...
- [ ] **AC-8** Non-goals hold: `git diff --name-only main...HEAD` (or `git diff --name-only` of the branch's changes) lists **neither** `.memex/scripts/validate-spec.sh` **nor** `skills/memex/scaffold/vault-scripts/validate-spec.sh`, and lists no **frozen spec** under `.memex/specs/` — i.e. no `.memex/specs/` path other than this spec's own `2026-06-18-memex-worktrees/` folder and the shared `_template/spec.md` (whose new `worktree:` field is an intended part of this change, per File Structure). The feature *implementation* rewrites no learning; the only `.memex/learnings/` path the diff may list is a reflection note from the delivery's reflect step (here, `validate-spec-flags-template-meta-specs.md`, refined with the keep-the-gate-green technique) — no other learning is modified.
