---
status: draft
feature: specward-pivot
scope: complex
created: 2026-06-24
shipped: null
branch: feat/specward
mode: autonomous
worktree: null
---
# specward Pivot — Spec
...
**Status:** Draft
**Design:** design.md
**Scope:** Rename the skill to specward/sw and strip it to a spec-driven-only workflow, removing the entire memory half and relocating all machinery into the skill.
...
1. **Rename** — `memex`→`sw` for every technical identifier (plugin name, command namespace, skill directories, scaffolder), `memex`→`specward` for the brand (vault directory `.specward/`, marketplace name, README, prose). Claude derives the command namespace from the plugin name, so the plugin must be named `sw` to yield `/sw:spec`.
...
Finally the repo re-hosts itself: `.memex/` is deleted and a fresh `.specward/{conventions,specs}` is born with this spec as record #1.

A target repo, post-install, receives exactly `AGENTS.md` + `.specward/{conventions,specs}`.
...
- `.specward/conventions/` (re-authored, no old-name references) and `.specward/specs/2026-06-24-specward-pivot/`

**Modified:**
- `plugins/sw/.claude-plugin/plugin.json` (`name: sw`), `.claude-plugin/marketplace.json` (marketplace `specward`, plugin `sw`, source `./plugins/sw`), `.claude/settings.json` (`sw@specward`)
...
6. **Phase 6 — Re-brand root docs** (README, install.sh, NOTICE, CONTRIBUTING to specward/sw).
7. **Phase 7 — Dogfood re-host** (delete `.memex/`, born-fresh `.specward/`, carry the pivot spec in as record #1, re-author conventions).
...
1. A developer runs `install.sh` in a fresh repo, then `/sw`; the scaffolder writes `AGENTS.md` + `.specward/{conventions,specs}` and enables the `sw` plugin — no learnings, no Obsidian, no `.specward/scripts`.
...
- [ ] **AC-4** `grep -rIl -e '\.obsidian' -e '\[\[' skills/sw plugins/sw .agents/skills AGENTS.md` returns zero files, and `grep -rIl '^related:' skills/sw/scaffold` returns zero files.
- [ ] **AC-5** `plugins/sw/.claude-plugin/plugin.json` has `"name": "sw"`; `.claude-plugin/marketplace.json` has marketplace `"name": "specward"` and a plugin entry with `"name": "sw"` and `"source": "./plugins/sw"`; `.claude/settings.json` enables `"sw@specward"`.
- [ ] **AC-6** The scaffolder (`skills/sw/SKILL.md` + `skills/sw/references/audit-checklist.md` + `skills/sw/references/validation.md`) names only `.specward/conventions/` and `.specward/specs/` as scaffolded vault directories — `grep -ril -e '\.specward/learnings' -e '\.specward/_index' -e '\.specward/templates' -e '\.specward/scripts' skills/sw` returns zero files.
- [ ] **AC-7** `skills/sw/scripts/validate-spec.sh` and `skills/sw/scripts/sw-update.sh` exist and are executable; `skills/sw/scaffold/vault-scripts/` no longer exists; the writing-plans and update SKILLs reference the `skills/sw/scripts/` path and no file instructs copying a script into `.specward/scripts`.
- [ ] **AC-8** Each of the three `code-review` SKILL.md copies contains the literal strings `Modularity`, `meaningful`, and `secret`, and `grep -L -e 'constitution' -e 'rules\.md'` confirms none of them reference the removed law files.
...
- [ ] **AC-11** `.memex` does not exist; `.specward/` contains exactly the entries `conventions` and `specs`; `.specward/specs/2026-06-24-specward-pivot/` contains `design.md`, `spec.md`, and `tasks.md`.
- [ ] **AC-12** `skills/sw/scripts/validate-spec.sh .specward/specs/2026-06-24-specward-pivot` exits 0, and the scaffolder validation routine (`skills/sw/scripts/quick_validate.py` or `references/validation.md` checks) reports no failures on this repo.
...
| Deleting `.memex/` loses spec history | git history retains everything; the pivot spec is carried into `.specward/specs/` |
