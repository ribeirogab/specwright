...
- [x] `git grep -l '\.vault' | grep -v '^\.memex/specs/'` returns ONLY files where the `.vault` token is the **intentional subject matter**, never a stale path: (a) the historical-narrative learning `.memex/learnings/sed-rename-pattern-completeness.md`, whose `.vault/` mentions describe the *result* of the prior `context → .vault/` rename — flipping them would state a falsehood; (b) the reflection-step learnings written for THIS spec that necessarily cite the old token as their subject — `.memex/learnings/rename-toward-overloaded-token-verifies-on-old-token.md` and `.memex/learnings/memex-link-copies-have-drifted.md` — plus their backtick-quoted entries in `.memex/_index/learnings.md`. Every other `.vault` survivor is under `.memex/specs/`: frozen shipped specs (bodies reference the ship-time directory name) plus this rename spec's own intentional narrative. Verified by running the command and confirming each survivor is a frozen spec, this spec's narrative, or a learning that is *about* the old token.
...
- [x] All `.memex/_index/`, `.memex/templates/`, `.memex/conventions/` files contain zero `.vault` *path* references. The only `.vault` token in this set is the backtick-quoted anchor token inside the `_index/learnings.md` entry for the new overloaded-token learning (subject matter, not a path). Verified by `git grep -F '.vault' .memex/templates/ .memex/conventions/` returning empty and the single `_index/learnings.md` hit being a code-quoted anchor-token reference.
