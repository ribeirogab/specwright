...
- [[../learnings/skills-cli-agent-selection-shapes-layout|skills CLI `-a <agent>` shapes the on-disk install layout]] — `-a universal` writes `.agents/skills/<name>/` (no symlink), `-a claude-code` copies into `.claude/skills/<name>/` (no `.agents/`), `*` adds a stray `agent/` dir; the memex canonical layout needs `-a universal` + a hand-made `.claude` symlink. From [[../specs/2026-06-24-install-script/spec|install-script]].
...
- [[../learnings/curl-pipe-sh-stdin-drain|`curl | sh`: the script is the shell's stdin — child readers drain it]] — under `curl … | sh`, a child that reads stdin (`npx`, an installer) consumes the rest of the script source, so trailing lines silently never run; fix with `</dev/null` on the command. Reproduce with `cat script.sh | sh`; verify a just-pushed installer by commit-SHA URL (branch raw is CDN-cached ~5 min).
