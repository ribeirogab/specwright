...
- **Expected:** following `skills/sw/SKILL.md` Phase 4 produces an install where the issue pipeline can run: the installed `sw-plan` skill (its "Mechanical" self-review gate) and the `/sw:review-spec` command both invoke the mechanical validator at `.agents/skills/sw/scripts/validate-spec.sh`, and validation check 11 expects the templates + validator to be present "inside this skill".
