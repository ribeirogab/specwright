...
- **[discovery]** Codex reads subagent role profiles from the **project** (`.codex/agents/*.toml`), and a plugin cannot supply them: an installed probe plugin declaring `"agents": "./codex-agents/"` in its manifest registered no role at all, while the same profiles placed in a project made `spawn_agent` expose an `agent_type` accepting `sw-change-owner` and `sw-reviewer`. Removing `.codex/` from initialized repositories is therefore not possible today. Claude Code has no equivalent constraint — it finds roles inside the installed plugin.
