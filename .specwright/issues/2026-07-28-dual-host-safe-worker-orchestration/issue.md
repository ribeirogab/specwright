...
- [x] **AC-7** The installed Codex profiles and Claude agent manifests expose the same four role names; owner and reviewer profiles select `gpt-5.6` with high reasoning, the worker selects `gpt-5.6-terra` with medium reasoning, and only the two reviewer profiles are read-only.
