...
- [ ] **Step 1: Create the evidence folder and the pre-test copies**
...
- [ ] **Step 2: Capture the pre-test git and file state**
...
- [ ] **Step 1: Start the transcript file**
...
- [ ] **Step 2: Spawn the session-under-test**
...
- [ ] **Step 3: Record and reply until the hold point**
...
- [ ] **Step 1: Capture the post-test state**
...
- [ ] **Step 2: Byte-level diffs against the pre-test copies**
...
- [ ] **Step 3: Sandbox code-integrity check**
...
- [ ] **Step 4: Commit the state evidence**
...
- [ ] **Step 1: Walk the transcripts for the AC-1/AC-2 checks**
...
- [ ] **Step 2: Record the AC-3 verdicts**
...
- [ ] **Step 2: Runtime verification mapping**

This is a test-execution issue: the drove-sessions ARE the runtime. Map each `AC-N` to its observed evidence (transcript lines, diff outputs) and tick the `issue.md` checkboxes only for ACs whose evidence shows a pass.

- [ ] **Step 3: Commit any final artifact touch-ups**
...
- [ ] **Step 1: Open the PR** — `/sw:pr`, base `chore/e2e-milestone-planning` (stacked on the milestone-planning branch; note it in the PR body). Never fabricate a PR URL.
- [ ] **Step 2: Review to lgtm** — `/sw:review`; fix blockers, re-run until `lgtm`.
- [ ] **Step 3: Curate learnings.md** — only non-obvious facts downstream issues (dispatch-parallelism T4, circuit-breaker T7, blocked-recovery T8) need: how sessions behave at the dispatch boundary, discovery quirks, harness caveats.
- [ ] **Step 4: Ship** — set `issue.md` `status: shipped` + date, tick verified ACs, commit on this branch.
