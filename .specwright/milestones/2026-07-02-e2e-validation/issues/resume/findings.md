...
- **O-2 — driver-name leak reproduced (T1 caveat confirmed); plan-vs-execution deviation acknowledged.** Both sessions saw the driver's agent identity — the NL session's closing message explicitly names "outra sessão sua (`owner-resume`)". This deviates from the spec's own mitigation ("drives directly — its name is not milestone-revealing"): `owner-resume` carries the issue slug, which the spec's non-leak intent should have excluded. On timing: the claim that the leak cannot reach the discovery turn is an *inference from the observed mechanism*, not a proven property — in T1 and here the identity surfaced through SendMessage sender attribution, and the spawn prompt itself contains no sender name; but the harness's spawn-time metadata is not inspectable from outside, so spawn-time visibility is not strictly ruled out. Two independent signals say the discovery was uncontaminated anyway: neither session's first turn references any caller identity, and both first turns derive strictly from the milestone files they quote. Future harnesses should spawn under a neutral driver name to make the question moot.
