...
- [x] Step 1: Confirm the sandbox HEAD is still `aaa117b` and the tree is clean
...
- [x] Step 2: Run the validator over each of the five fixture issue folders, capturing output
...
- [x] Step 3: Extract each ticket's frontmatter, slug shape, and AC list into the evidence file
...
- [x] Step 1: Read `$FIXTURE/goal.md` in full; record section inventory (Purpose, Motivation, Success Criteria, Non-Goals) and sweep for technical content
...
- [x] Step 3: Write the AC-1 and AC-2 verdict notes (PASS/FAIL + evidence line refs) into working notes for findings.md
...
- [x] Step 1: Read each of the five `issue.md` files in full; per ticket judge: frontmatter `status: pending`, AC bullets numbered sequentially and binary (observable, yes/no-checkable), no surviving double-brace template placeholders
- [x] Step 2: Append the slug-shape check to the evidence file
...
- [x] Step 3: Write the AC-3 verdict notes: per-ticket PASS/FAIL with the validator baseline interpretation (check-2 line expected, anything else a divergence)
...
- [x] Step 1: Read `milestone-session.md` in full; locate (a) the post-design batch, (b) the handoff, (c) the transcript tail after the handoff
- [x] Step 2: For each of the three contract points, record the verdict with its evidence tier — `verbatim` when the evidence file quotes session text, `relay` when it summarizes; relay evidence yields "consistent with the contract (relay)" not "proven"
- [x] Step 3: Check the tail for conduction signs (dispatch language, sub-agent spawns, code edits); record PASS/FAIL + tier
...
- [x] Step 1: Write `$ISSUE/findings.md`: header stating fixture commit + evidence provenance (incl. the relay-fidelity note and the validator-baseline interpretation), then one section per check (goal.md, board.md, tickets, transcript) with PASS/FAIL verdicts and one Expected / Observed / Proposed fix block per failure; proposed fixes name the upstream skill/template, never the fixture
- [x] Step 2: Verify completeness mechanically
...
- [x] Step 3: Run the mechanical gate on this issue's own folder

Run: `skills/sw/scripts/validate-spec.sh "$ISSUE"`
Expected: `PASS` (exit 0).

- [x] Step 4: Commit the audit artifacts on `chore/e2e-milestone-planning`
