...
A **driver/driven harness**, same shape as scope-detection (T1) and resume (T2): this session (the driver) spawns one neutrally-named general-purpose sub-agent (the **driven session**) whose cwd is the sandbox, plays the user with scripted replies over SendMessage, polls repository state between turns, and captures key turns verbatim with `jq` from the sub-agent's JSONL transcript. The driver never edits sandbox files; every sandbox mutation must come from the driven session.

**Driven-session contract (folds in every applicable inherited learning):**

- **Neutral name** — `maintainer9` (leak mechanism reproduced 4x: slug-bearing names contaminate via SendMessage attribution; the spawn prompt itself carries no sender name).
...
4. Post-design batch: answer whatever the session asks in one reply (branch name: accept its suggestion; worktree: yes; handoff: follow what the session itself offers — continue in-session or hand off to `/sw:plan`; either is valid, record which; on a handoff, spawn a second driven session (`maintainer10`, same contract) in the issue's checkout to run the pipeline).
...
2. **Brainstorm drive** — spawn `maintainer9`, run the design conversation to approval, scope, batch. Depends on 1.
...
| Driver name/expected-outcome leak contaminates the run | Neutral names (`maintainer9`/`maintainer10`); pre-send checklist over every reply against the forbidden-terms list; spawn prompt carries no expectations |
