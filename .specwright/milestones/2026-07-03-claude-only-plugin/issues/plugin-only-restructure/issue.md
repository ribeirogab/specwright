...
- Editing `README.md` or the repo's root entry-point document — owned by `docs-and-install-flow`.
...
- [ ] **AC-1** Under `plugins/sw/` there is exactly one copy of: the five artifact templates (`issue`, `spec`, `tasks`, `goal`, `board`), `validate-spec.sh` with its `fixtures/` directory, and the reference docs — each present as a file at a path inside `plugins/sw/`.
- [ ] **AC-2** The directories `.agents/` and `skills/sw/` do not exist anywhere in the repository tree.
- [ ] **AC-3** `grep -rn` over the whole repository for the literal strings `.agents/skills` and `scaffold/skills` returns zero matches.
- [ ] **AC-4** Each bundled companion skill (`brainstorm`, `plan`, `pr`, `review`, `run`) has exactly one `SKILL.md` on disk, located under `plugins/sw/skills/`; no second copy of any of those `SKILL.md` files exists elsewhere in the repository.
- [ ] **AC-5** Every template path and validator path referenced inside the `plan` and `brainstorm` skill files resolves to an existing file under `plugins/sw/` (verified by listing each referenced path and confirming it exists).
