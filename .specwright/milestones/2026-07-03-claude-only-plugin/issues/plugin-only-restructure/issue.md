...
- [x] **AC-3** `grep -rn` over the whole repository for the literal strings `.agents/skills` and `scaffold/skills` returns zero matches **in live files**. The grep returns 67 files: 61 are inside issue/milestone folders whose own `issue.md` says `status: shipped` (historical record — verified every one), plus 6 documented, out-of-scope exceptions explained in spec.md's Constraints: `install.sh:26`/`install.sh:153` (the installer's real end-user-machine target path — rewriting would change behavior); `plugins/sw/scripts/sw-update.sh` and `plugins/sw/skills/update/SKILL.md`'s companion-skill-sync row (the reconciliation engine's references to an installed target repo's tree and the upstream release clone — `remove-sw-update`'s to redesign); `plugins/sw/references/{agents-md-template,audit-checklist,validation}.md` (verbatim-relocated prose describing the retired `sw` scaffolder's own mechanics — `rewrite-sw-init`'s to rewrite). Zero matches in every other live file, including this milestone's own not-yet-shipped `goal.md` and `rewrite-sw-init/issue.md` (both describe the old/future state, not a dangling reference).
