...
- **`plugins/sw/skills/update/SKILL.md`** references `.agents/skills/sw/scripts/sw-update.sh` and `.agents/skills/sw/.update-manifest.json` — these are `remove-sw-update`'s files to delete entirely per the board dependency graph (`remove-sw-update` depends on `plugin-only-restructure`). This issue does not touch `plugins/sw/skills/update/SKILL.md`'s content or delete `skills/sw/scripts/sw-update.sh` — but since `sw-update.sh` lives under `skills/sw/` which this issue deletes, the file's physical deletion is an unavoidable side effect of collapsing `skills/sw/`. The **remaining reference to it** (inside `plugins/sw/skills/update/SKILL.md`, naming `.agents/skills/sw/scripts/sw-update.sh`) is explicitly `remove-sw-update`'s to clean up (that issue's AC-2/AC-3 cover exactly this), and it is **not** a template/validator/brainstorm/plan reference this issue's AC-3/AC-5 constrain — AC-3 is a strings-only grep with no exception, so this file's `.agents/skills` mention would trip it. Resolution: `remove-sw-update` is a sibling issue not yet shipped, so its file cannot be assumed fixed; this issue must not leave `.agents/skills` inside `plugins/sw/skills/update/SKILL.md` either. The minimal fix consistent with "not touching runtime behavior" is to update just the two path mentions in that file to `plugins/sw/scripts/sw-update.sh` and `plugins/sw/.update-manifest.json` (mechanical path substitution, same shape as the other rewires), leaving `remove-sw-update` to delete the file and the skill wholesale later.
...
- **AC-3 verification output** — because of the two `install.sh` survivors above, the AC-3 checkbox in `issue.md` cannot be ticked as a bare "zero matches" fact. It is ticked `[x]` with an inline note: "verified — grep returns only `install.sh:26` and `install.sh:153`, both the installer's real end-user-machine target path, explicitly out of scope per spec.md's Constraints; zero matches in every other file." This keeps Rule of Repair / Rule of Transparency: the exception is loud, not silent.
...
| Deleting `skills/sw/scripts/sw-update.sh` breaks `plugins/sw/skills/update/SKILL.md`'s own instructions before `remove-sw-update` ships. | Rewire (not delete) the two path mentions in `update/SKILL.md` to `plugins/sw/scripts/sw-update.sh`; leave the file's removal to `remove-sw-update`, whose AC-1/AC-2 own it. |
