...
- [ ] `find . -type d -name 'vault' -not -path './.git/*' -not -path './node_modules/*'` returns exactly two paths: `./vault` and `./.agents/skills/memex-link/tests/fixtures/vault` and `./skills/memex/scaffold/skills/memex-link/tests/fixtures/vault`. Verified by listing.
- [ ] `git grep -l 'context/'` returns ONLY the three shipped spec folders' files (`vault/specs/2026-04-30-opensource-readiness/*.md`, `vault/specs/2026-05-03-rename-harness-to-memex/*.md`, `vault/specs/2026-05-03-strengthen-vault-cross-links/*.md`) and any per-occurrence-reviewed survivors in mixed-context learnings. Each survivor is annotated in the commit message with a one-line justification.
